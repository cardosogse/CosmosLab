...
def inicializar_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS tokens_acceso
                 (token TEXT PRIMARY KEY, fecha_expiracion DATE, en_uso BOOLEAN, 
                  identificador_usuario TEXT, modulo_actual INTEGER DEFAULT 1,
                  score_puntos INTEGER DEFAULT 0, memo_completado INTEGER DEFAULT 0)''')
    token_prueba = "SYNAPSIS-PRO-2026"
    fecha_futura = datetime.date.today() + timedelta(days=30)
    c.execute("INSERT OR IGNORE INTO tokens_acceso VALUES (?, ?, 0, 'Admin', 1, 0, 0)", 
              (token_prueba, fecha_futura))
    conn.commit()
    conn.close()
...
def registrar_nuevo_usuario(token, dias_duracion, identificador="Nuevo Estudiante"):
    ...
    try:
        ...
        mensaje = "Error: El token ya existe en el servidor."
    conn.close()
    return mensaje
