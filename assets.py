...
def inyectar_css():
    st.markdown("""
    <style>
        .stApp { background-color: #000000 !important; background-image: radial-gradient(white 1px, transparent 1px), radial-gradient(white 1px, transparent 1px); background-size: 250px 250px, 150px 150px; background-position: 0 0, 30px 40px; }
        ...
        div[data-testid="stRadio"] label { background-color: rgba(255, 255, 255, 0.05) !important; border: 1px solid rgba(255, 255, 255, 0.1) !important; padding: 10px 18px !important; border-radius: 20px !important; color: #cfd8dc !important; transition: all 0.2s ease-in-out !important; cursor: pointer !important; }
        div[data-testid="stRadio"] label:hover { background-color: rgba(0, 229, 255, 0.08) !important; border-color: #00e5ff !important; color: #ffffff !important; }
        div[data-testid="stRadio"] label[data-checked="true"] { background-color: rgba(0, 229, 255, 0.18) !important; border-color: #00e5ff !important; color: #00e5ff !important; font-weight: bold !important; }
        div[data-testid="stRadio"] label div[data-testid="stMarkdownContainer"]-prefix { display: none !important; }
        div[data-testid="stRadio"] div[role="radiogroup"] > label > div:first-child { display: none !important; }

        div[data-testid="stSlider"] { cursor: pointer !important; }
        div[data-testid="stSlider"] [role="slider"] { cursor: grab !important; }
        div[data-testid="stSlider"] [role="slider"]:active { cursor: grabbing !important; }
        div[data-testid="stElementToolbar"] + div div[data-baseweb="slider-track"] > div { background: linear-gradient(to right, #00e5ff, #00e5ff) !important; }
        div[data-testid="stElementToolbar"] + div [role="slider"] { background-color: #00e5ff !important; box-shadow: 0 0 8px #00e5ff !important; }

        @keyframes parpadeoPulso { 0% { opacity: 0.3; text-shadow: 0 0 0px transparent; } 50% { opacity: 1; text-shadow: 0 0 8px #ffb142; } 100% { opacity: 0.3; text-shadow: 0 0 0px transparent; } }
        .foco-parpadeante { animation: parpadeoPulso 2.5s infinite ease-in-out; color: #ffb142; font-weight: bold; display: inline-block; }

        .card-dalton { background-color: rgba(144, 164, 174, 0.08); border-left: 5px solid #90a4ae; padding: 20px; border-radius: 6px; margin-bottom: 15px; }
        .card-thomson { background-color: rgba(156, 39, 176, 0.08); border-left: 5px solid #9c27b0; padding: 20px; border-radius: 6px; margin-bottom: 15px; }
        .card-rutherford { background-color: rgba(33, 150, 243, 0.08); border-left: 5px solid #2196f3; padding: 20px; border-radius: 6px; margin-bottom: 15px; }
        .card-bohr { background-color: rgba(255, 177, 66, 0.08); border-left: 5px solid #ffb142; padding: 20px; border-radius: 6px; margin-bottom: 15px; }
        .card-schrodinger { background-color: rgba(0, 229, 255, 0.08); border-left: 5px solid #00e5ff; padding: 20px; border-radius: 6px; margin-bottom: 15px; }
    </style>
...
def generar_svg_tira_afloja(f1, c1, sym1, f2, c2, sym2):
    diff = abs(f1 - f2)
    cx_e = 120 if diff == 0 else (80 + (1.0 / diff) * 5 if f1 > f2 else 160 - (1.0 / diff) * 5)
    ellipse_w = 40 if diff == 0 else 55
    stroke_color = "#ffffff" if diff == 0 else (c1 if f1 > f2 else c2)
...
def generar_svg_enlace(sym1, f1, c1, sym2, f2, c2):
    diff = abs(f1 - f2)
    cx_e1, cx_e2, ellipse_x, ellipse_w, stroke_color, stroke_dash = (113, 127, 120, 65, "#ffffff", "2 2") if diff == 0 else ((85, 95) if f1 > f2 else (145, 155), (100, 70) if f1 > f2 else (140, 70), c1 if f1 > f2 else c2, "4 2") if diff > 0.4 else (105, 135, 120, 68, "#b0bec5", "3 3")
    return f"""
    <div style='display:flex; justify-content:center; align-items:center; width:100%; height:130px;'>
        <svg viewBox="0 0 240 120" width="100%" height="100%">
            ...
            <circle cx="{cx_e1}" cy="60" r="4" fill="#ffffff"/><circle cx="{cx_e2}" cy="60" r="4" fill="#ffffff"/>
        </svg>
    </div>
    """
